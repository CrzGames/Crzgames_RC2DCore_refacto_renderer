...
if [[ ! -f "${CACHE_FILE}" ]]; then
  ...
  echo "[INFO ] Run first: ./build-scripts/generate-project/ios-iphoneos-arm64-signedbundleapp.sh"
  exit 1
fi

echo -e "\e[32m[INFO ] Rebuilding Debug configuration (iOS iphoneos arm64, signing enabled)...\e[0m"
if [[ -n "${TARGET}" ]]; then
  targets=("${TARGET}")
else
