...
if [[ ! -f "${CACHE_FILE}" ]]; then
  ...
  echo "[INFO ] Run first: ./build-scripts/generate-project/macos-arm64-nosignedbundleapp.sh"
  exit 1
fi

echo -e "\e[32m[INFO ] Rebuilding Debug configuration (macOS arm64, no signing)...\e[0m"
if [[ -n "${TARGET}" ]]; then
  targets=("${TARGET}")
else
